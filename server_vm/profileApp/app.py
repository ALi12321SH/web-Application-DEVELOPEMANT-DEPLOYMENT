...
app.secret_key = '40010743ali'


app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = '40010743ali'
...
def login():
	...
	if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
		...
		if username == 'admin' and password == 'admin':
			...
			return render_template('home.html', accounts = account)
		cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(
...
def delete_account():
	db = MySQLdb.connect(host="localhost", user="root", passwd="40010743ali", db="profileApp")
	cursor = db.cursor()
	session_id = session['id']
	cursor.execute("DELETE FROM accounts WHERE id = % s", (session_id,))
	db.commit()
